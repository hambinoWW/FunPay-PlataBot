...
$product""")  # todo
    ...
    def open_gf_list(c: CallbackQuery):
        ...
        bot.edit_message_text(_("desc_gf"), c.message.chat.id, c.message.id,
                              reply_markup=kb.products_files_list(offset, crd.products_directory))
        bot.answer_callback_query(c.id)
    ...
    def create_gf(m: Message):
        ...
        file_name += ".txt"
        if os.path.exists(crd.product_path(file_name)):
            file_index = os.listdir(crd.products_directory).index(file_name)
        ...
        try:
            with open(crd.product_path(file_name), "w", encoding="utf-8"):
                pass
        except:
    ...
    def link_gf(m: Message):
        ...
        file_name += ".txt"
        if not os.path.exists(crd.product_path(file_name)):
    ...
    def open_gf_settings(c: CallbackQuery):
        ...
        files = [i for i in os.listdir(crd.products_directory) if i.endswith(".txt")]
        if not check_products_file_exists(file_index, files, c.message, reply_mode=False):
            bot.answer_callback_query(c.id)
            return

        file_name = files[file_index]
        products_amount = cardinal_tools.count_products(crd.product_path(file_name))
        nl = "\n"
...
<i>{_('gl_last_update')}:</i>  <code>{datetime.datetime.now().strftime('%H:%M:%S')}</code>"""
    ...
    def add_products_to_file(m: Message):
        ...
        try:
            with cardinal_tools.get_products_file_lock(crd.product_path(file_name)):
                with open(crd.product_path(file_name), "a", encoding="utf-8") as f:
                    f.write("\n")
    ...
    def send_products_file(c: CallbackQuery):
        ...
        file_name = files[file_index]
        with cardinal_tools.get_products_file_lock(crd.product_path(file_name)):
            with open(crd.product_path(file_name), "r", encoding="utf-8") as f:
                data = f.read().strip()
                if not data:
    ...
    def del_products_file(c: CallbackQuery):
        ...
        try:
            with cardinal_tools.get_products_file_lock(crd.product_path(file_name)):
                os.remove(crd.product_path(file_name))
