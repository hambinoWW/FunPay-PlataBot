...
def init_config_loader_cp(cardinal: Cardinal, *args):
    ...
    def send_config(c: types.CallbackQuery):
        ...
        if config_type == "main":  # locale
            ...
            path, text = getattr(cardinal, "main_config_path", "configs/_main.cfg"), _("cfg_main")
        elif config_type == "autoResponse":
            path, text = getattr(cardinal, "auto_response_config_path", "configs/auto_response.cfg"), _("cfg_ar")
        elif config_type == "autoDelivery":
            path, text = getattr(cardinal, "auto_delivery_config_path", "configs/auto_delivery.cfg"), _("cfg_ad")
        else:
            bot.answer_callback_query(c.id)
            return
