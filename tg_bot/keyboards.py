...
def products_files_list(offset: int, products_directory: str = "storage/products") -> K:
    """
    ...
    if not files and offset != 0:
        offset = 0
        files = os.listdir(products_directory)[offset:offset + 5]

    for index, name in enumerate(files):
        amount = Utils.plata_tools.count_products(os.path.join(products_directory, name))
...
def edit_lot(c: Cardinal, lot_number: int, offset: int) -> K:
    ...
    if not file_name:
        ...
        if file_name not in os.listdir(c.products_directory):
            with open(c.product_path(file_name), "w", encoding="utf-8"):
                pass
        file_number = os.listdir(c.products_directory).index(file_name)
