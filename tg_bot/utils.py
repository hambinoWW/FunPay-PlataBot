...
from plata_identity import PLATA_OWNER_TELEGRAM_ID
...
def get_plata_owner_id(authorized_users: dict[int, dict] | None = None) -> int | None:
    """Return the fixed PLATA owner ID."""
    return PLATA_OWNER_TELEGRAM_ID
...
def has_brand_mark(watermark: str) -> bool:
    ...
    ascii_hits = any(kw in simplified for kw in ("cardinal", "fpc", "plata"))
    ...
    if lot_obj.get("productsFileName") is None:
        ...
        if not os.path.exists(f"storage/products/{lot_obj.get('productsFileName')}"):
            with open(f"storage/products/{lot_obj.get('productsFileName')}", "w", encoding="utf-8"):
                pass
        products_amount = plata_tools.count_products(f"storage/products/{lot_obj.get('productsFileName')}")
