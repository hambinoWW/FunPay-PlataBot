...
def init_uploader(cardinal: Cardinal):
    ...
    def upload_products_file(m: types.Message):
        ...
        if not check_file(tg, m, type_="txt"):
            return
        products_directory = getattr(cardinal, "products_directory", "storage/products")
        products_path = cardinal.product_path(m.document.file_name)
        with cardinal_tools.get_products_file_lock(products_path):
            if not download_file(tg, m, m.document.file_name,
                                 custom_path=products_directory):
                return

        try:
            products_count = cardinal_tools.count_products(cardinal.product_path(utils.escape(m.document.file_name)))
        except:
            bot.send_message(m.chat.id, "❌ Произошла ошибка при подсчете товаров.")
            logger.debug("TRACEBACK", exc_info=True)
            return

        file_number = os.listdir(products_directory).index(m.document.file_name)
    ...
    def temp_config_name(m: types.Message, kind: str) -> str:
        account_id = getattr(cardinal, "account_profile_id", "primary")
        safe_account_id = "".join(ch for ch in account_id if ch.isalnum() or ch in "_-")
        return f"temp_{kind}_{safe_account_id}_{m.from_user.id}.cfg"
    ...
    def upload_main_config(m: types.Message):
        ...
        if not check_file(tg, m, type_="cfg"):
            return
        temp_name = temp_config_name(m, "main")
        temp_path = f"storage/cache/{temp_name}"
        if not download_file(tg, m, temp_name):
            return
    ...
    def upload_auto_response_config(m: types.Message):
        ...
        if not check_file(tg, m, type_="cfg"):
            return
        temp_name = temp_config_name(m, "auto_response")
        temp_path = f"storage/cache/{temp_name}"
        if not download_file(tg, m, temp_name):
            return
        ...
        bot.send_message(m.chat.id, "🔁 Проверяю валидность файла...")
        try:
            new_config = cfg_loader.load_auto_response_config(temp_path)
            raw_new_config = cfg_loader.load_raw_auto_response_config(temp_path)
        except excs.ConfigParseError as e:
    ...
    def upload_auto_delivery_config(m: types.Message):
        ...
        if not check_file(tg, m, type_="cfg"):
            return
        temp_name = temp_config_name(m, "auto_delivery")
        temp_path = f"storage/cache/{temp_name}"
        if not download_file(tg, m, temp_name):
            return
        ...
        try:
            new_config = cfg_loader.load_auto_delivery_config(
                temp_path,
                getattr(cardinal, "products_directory", "storage/products"),
            )
        except excs.ConfigParseError as e:
    ...
    def upload_plugin(m: types.Message):
        ...
        bot.send_message(m.chat.id,
                         ...
                         f"⚠️ Чтобы плагин активировался, <u><b>перезагрузите PLATA.</b></u> (/restart)",
                         reply_markup=keyboard)
