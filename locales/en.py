...
pl_install_deps = "📦 Install libraries"
pl_deps_installing = "⏳ Installing libraries: <code>{}</code>. It may take a couple of minutes."
pl_deps_installed = "✅ Libraries installed: <code>{}</code>.\n\n{}"
pl_deps_install_err = "❌ Failed to install libraries <code>{}</code>.\n\n<code>{}</code>"
pl_deps_nothing = "✅ All of the plugin libraries are already available."
pl_deps_no_output = "pip returned no output."
pl_deps_restart = "Restart PLATA (/restart) so the plugin loads with the new library."
pl_missing_deps_notify = "📦 <b>The «{}» plugin is missing a library</b>\n\nInstall: <code>{}</code>"
...
access_granted = "🔓 Access granted!\n\n" \
                 ...
                 "⚙️ To open the <i>PLATA</i> settings menu, send me /menu."
...
✅ <i>PLATA</i> gets the full chat history and sees all data about all new messages.
✅ <i>PLATA</i> can see images in chat and forward them to <i>Telegram</i> chat.
✅ <i>PLATA</i> can determine exactly who wrote, whether it was you, your interlocutor, or a 3rd party (arbitrator).
❌ Because <i>PLATA</i> gets the full chat history to detect new messages, the chat becomes "read" (not lit orange).
...
❌ <i>PLATA</i> doesn't get the full chat history, so it sees only the last message. If the user quickly writes several messages, <i>PLATA</i> will see only the last one.
❌ <i>PLATA</i> cannot see images in chat and forward them to <i>Telegram</i> chat.
❌ <i>PLATA</i> cannot determine exactly who wrote: you or the person you are chatting with. If the chat is not read, then the message is from the interlocutor, otherwise it is from you. However, if you are viewing the chat when you receive messages, this logic can sometimes fail. Also, <i>PLATA</i> will not be able to determine if a 3rd party (arbitrator) wrote into the chat.

❗ If you click the <code>More</code> button in a new message notification, <i>PLATA</i> will "read" the chat and show the last 15 messages, including images. <i>PLATA</i> will also be able to determine who the author of the messages is."""
...
❌ <i>PLATA</i> is not initialized yet and none of its functions work.\n
🔃 As soon as <i>PLATA</i> is initialized, this message will change.\n
📋 If <i>PLATA</i> does not initialize for a long time, check the logs with /logs"""

plata_init = """✅ <b><u>PLATA initialized!</u></b>\n
ℹ️ <b><i>Version:</i></b> <code>{}</code>
👑 <b><i>Account:</i></b>  <code>{}</code> | <code>{}</code>
💰 <b><i>Balance:</i></b> <code>{}₽, {}$, {}€</code>
📊 <b><i>Active orders:</i></b>  <code>{}</code>

💬 <b><i>Telegram chat:</i></b> @funpayplata
🔄 <b><i>Updates:</i></b> @funpayplata
🧩 <b><i>Plugins:</i></b> @quantumdeals
👨‍💻 <b><i>Developer:</i></b> @hambinoww
🤑 <b><i>Donate:</i></b> @hambinoww"""
...
<b>CPU:</b>
{}
    Used by <i>PLATA</i>: <code>{}%</code>

<b>RAM:</b>
    Total:  <code>{} MB</code>
    Used:  <code>{} MB</code>
    Free:  <code>{} MB</code>
    Used by <i>PLATA</i>:  <code>{} MB</code>
...
update_lasted = "✅ You have the latest version PLATA {}"
...
update_done = "✅ The update is installed! Restart the PLATA with the /restart command."
update_done_exe = "✅ The update is installed! New <code>PLATA.exe</code> is in <code>update</code> folder. " \
                  "Turn off <i>PLATA</i>, replace old <code>PLATA.exe</code> with new one and run <code>Start.bat</code>. "
...
pl_new = "Send me a plugin.\n\n<b>⚠️ ATTENTION! Downloading plugins from questionable sources may lead to unfortunate consequences.\n" \
         "@quantumdeals solves most potential issues.</b>"

au_user_settings = "Settings for user {}"
adv_fpc = "😎 PLATA - the best bot for FunPay"
...
🛠️ Create your own bot: PLATA project
🔄 Updates: @funpayplata
🧩 Plugins: @quantumdeals
💬 Chat: @funpayplata"""

# - Menus desc
desc_main = "Select a settings category."
desc_more = "Additional PLATA tools."
desc_lang = desc_main
desc_gs = "Here you can turn the basic <i>PLATA</i> functions on and off."
...
cmd_stats = "PLATA sales analytics"
cmd_accounts = "FunPay accounts"
cmd_stats_all = "analytics for all accounts"
...
cmd_restart = "restart PLATA"
cmd_power_off = "shutdown PLATA"
cmd_analytics_report = "extended analytics"
cmd_analytics = "sales and customer analytics"
cmd_plugin_accounts = "assign accounts to plugin"
cmd_plugin_copy = "create a plugin backup"
cmd_plugin_recover = "restore a plugin backup"
cmd_plugin_access = "select accounts for a plugin"
cmd_plugin_help = "plugin management help"
cmd_reminder_text = "change the order reminder text"
cmd_old_orders = "find open orders older than one day"
cmd_profile_plus = "extended profile statistics"
cmd_review_reply = "configure review replies"

cmd_plugins_audit = "audit plugins"
cmd_plugin_backup = "back up a plugin"
cmd_plugin_restore = "restore a plugin"
...
exc_balance_get_err = "Failed to get the balance: the account has no available lots."
...
log_pl_deps_installed = "$MAGENTA@{} (ID: {})$RESET installed libraries for the $YELLOW\"{}\"$RESET plugin: $CYAN{}$RESET."
log_pl_deps_install_err = "Failed to install libraries for the $YELLOW\"{}\"$RESET plugin: {}"
...
crd_balance_get_warn = "Could not fetch the account balance (no available lots). The balance is shown as 0 — refresh it with the «🔄 Refresh» button in the profile."
