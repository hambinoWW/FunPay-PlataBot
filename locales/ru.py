...
pl_install_deps = "📦 Установить библиотеки"
pl_deps_installing = "⏳ Устанавливаю библиотеки: <code>{}</code>. Это может занять пару минут."
pl_deps_installed = "✅ Библиотеки установлены: <code>{}</code>.\n\n{}"
pl_deps_install_err = "❌ Не удалось установить библиотеки <code>{}</code>.\n\n<code>{}</code>"
pl_deps_nothing = "✅ Все библиотеки плагина уже доступны."
pl_deps_no_output = "pip не вернул вывод."
pl_deps_restart = "Перезапустите PLATA (команда /restart), чтобы плагин загрузился с новой библиотекой."
pl_missing_deps_notify = "📦 <b>Плагину «{}» не хватает библиотеки</b>\n\nНужно установить: <code>{}</code>"
...
access_granted = "🔓 Доступ к ПУ предоставлен!\n\n" \
                 ...
                 "⚙️ Чтобы открыть меню настроек <i>PLATA</i>, введи команду /menu."
...
✅ <i>PLATA</i> получает полную историю чата и видит все данные о всех новых сообщениях.
✅ <i>PLATA</i> может видеть изображения в чате и пересылать их в <i>Telegram</i> чат.
✅ <i>PLATA</i> может точно определить, кто именно написал, будь то ты, твой собеседник или 3е лицо (арбитраж).
❌ Из-за того что <i>PLATA</i> получает полную историю чата для обнаружения новых сообщений, чат становится "прочитанным" (не загорается оранжевым).
...
❌ <i>PLATA</i> не получает полную историю чата, вследствие чего видит только последнее сообщение, а не все. Если пользователь быстро напишет несколько сообщений, <i>PLATA</i> увидит только последнее.
❌ <i>PLATA</i> не может видеть изображения в чате и пересылать их в <i>Telegram</i> чат.
❌ <i>PLATA</i> не может точно определить, кто именно написал: ты или собеседник. Если чат не прочитан, значит сообщение от собеседника, иначе - от тебя. Однако, если ты смотришь чат в момент получения сообщений, данная логика иногда может сбоить. Так же <i>PLATA</i> не сможет определить, что в чат написало 3е лицо (арбитраж).

❗ Если нажать кнопку <code>Больше</code> в уведомлении о новом сообщении, <i>PLATA</i> "прочитает" чат и покажет последние 15 сообщений, в том числе изображения. Так же <i>PLATA</i> сможет определить, кто именно автор сообщений."""
...
❌ <i>PLATA</i> еще <b><u>не инициализирован</u></b> и <b><u>никакие его функции не работают</u></b>.\n
🔃 Как только <i>PLATA</i> инициализируется, данное сообщение изменится.\n
📋 Если <i>PLATA</i> долго не инициализируется, проверь логи с помощью команды /logs"""

plata_init = """✅ <b><u>PLATA инициализирована!</u></b>\n
ℹ️ <b><i>Версия:</i></b> <code>{}</code>
👑 <b><i>Аккаунт:</i></b>  <code>{}</code> | <code>{}</code>
💰 <b><i>Баланс:</i></b> <code>{}₽, {}$, {}€</code>
📊 <b><i>Активные заказы:</i></b>  <code>{}</code>

💬 <b><i>Telegram чат:</i></b> @funpayplata
🔄 <b><i>Обновления:</i></b> @funpayplata
🧩 <b><i>Плагины:</i></b> @quantumdeals
👨‍💻 <b><i>Разработчик:</i></b> @hambinoww
🤑 <b><i>Донат:</i></b> @hambinoww"""
...
about = """<b>🦊 PLATA 🦊 v{}</b>
<b>Обновления:</b> @funpayplata
<b>Плагины:</b> @quantumdeals
<b>Разработчик:</b> @hambinoww"""

sys_info = """<b><u>Сводка данных</u></b>

<b>ЦП:</b>
{}
    Используется <i>PLATA</i>: <code>{}%</code>
...
proxy_undeletable_account = "❌ Прокси нельзя удалить: он используется аккаунтом {}."
...
update_lasted = "✅ У тебя стоит последняя версия PLATA {}"
...
update_done = "✅ Установил обновление! Перезапусти <i>PLATA</i> с помощью команды /restart"
update_done_exe = "✅ Установил обновление! Новый <code>PLATA.exe</code> находится в папке <code>update</code>. " \
                  "Выключи <i>PLATA</i>, перенеси новый <code>PLATA.exe</code> на место старого " \
...
pl_new = "Отправь мне плагин.\n\n<b>⚠️ ВНИМАНИЕ! Загрузка плагинов из сомнительных источников может привести к печальным последствиям.\n" \
         "@quantumdeals решает большинство возможных проблем.</b>"

au_user_settings = "Настройки для пользователя {}"
adv_fpc = "😎 PLATA - лучший бот для FunPay"
adv_description = """🦊 PLATA v{} 🦊
...
🛠️ Создать своего бота: PLATA project
💰 Донат: @hambinoww
🔄 Обновления: @funpayplata
🧩 Плагины: @quantumdeals
💬 Чат: @funpayplata"""

# - Описания меню
desc_main = "Выбери категорию настроек."
desc_more = "Дополнительные инструменты PLATA."
desc_lang = desc_main
desc_gs = "Здесь ты можешь включать и выключать основные функции <i>PLATA</i>."
...
prx_account = "Аккаунт: <b>{}</b> (<code>{}</code>)"

# - Описание команд
cmd_menu = "открыть настройки"
cmd_language = "изменить язык"
cmd_profile = "статистика аккаунта"
cmd_stats = "аналитика продаж PLATA"
cmd_accounts = "аккаунты FunPay"
cmd_stats_all = "общая аналитика всех аккаунтов"
...
cmd_restart = "перезапустить PLATA"
cmd_power_off = "выключить PLATA"
cmd_analytics_report = "расширенная аналитика"
cmd_analytics = "аналитика продаж и покупателей"
cmd_plugins_audit = "проверить плагины"
cmd_plugin_backup = "создать копию плагина"
cmd_plugin_restore = "восстановить плагин"
cmd_plugin_accounts = "назначить аккаунты плагину"
cmd_plugin_copy = "создать копию плагина"
cmd_plugin_recover = "восстановить плагин из копии"
cmd_plugin_access = "выбрать аккаунты для плагина"
cmd_plugin_help = "справка по управлению плагинами"
cmd_reminder_text = "изменить текст напоминания о заказе"
cmd_old_orders = "найти открытые заказы старше суток"
cmd_profile_plus = "расширенная статистика профиля"
cmd_review_reply = "настроить ответ на отзыв"
...
exc_balance_get_err = "Не удалось получить баланс: у аккаунта нет доступных лотов."
...
log_pl_deps_installed = "$MAGENTA@{} (ID: {})$RESET установил библиотеки для плагина $YELLOW\"{}\"$RESET: $CYAN{}$RESET."
log_pl_deps_install_err = "Не удалось установить библиотеки для плагина $YELLOW\"{}\"$RESET: {}"
...
crd_balance_get_warn = "Не удалось получить баланс аккаунта (нет доступных лотов). Баланс показан как 0 ₽ — обновить можно кнопкой «🔄 Обновить» в профиле."
