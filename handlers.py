...
logger = logging.getLogger("PLATA.handlers")
localizer = Localizer()
...
def send_new_msg_notification_handler(c: Cardinal, e: NewMessageEvent) -> None:
    ...
    for i in events:
        ...
        if i.message.author_id == last_message_author_id and i.message.by_bot == last_by_bot and \
            ...
            author = f"<i><b>🤖 {_('you')} (<i>PLATA</i>):</b></i> " if i.message.by_bot else f"<i><b>🫵 {_('you')}:</b></i> "
...
def log_new_order_handler(c: Cardinal, e: NewOrderEvent, *args):
    ...
    account_id = getattr(c, "account_profile_id", "primary")
    if plata_analytics.record_order(e.order, account_id):
        logger.info(f"Заказ $YELLOW#{e.order.id}$RESET добавлен в аналитику PLATA.")
...
def update_plata_analytics_handler(cardinal: Cardinal, event: OrderStatusChangedEvent):
    """Updates PLATA analytics when an order is closed or refunded."""
    account_id = getattr(cardinal, "account_profile_id", "primary")
    plata_analytics.update_order_status(event.order, account_id)
...
def send_bot_started_notification_handler(c: Cardinal, *args):
    ...
    if c.telegram is None:
        return
    text = ("<b>🟠 PLATA · онлайн</b>\n"
            f"📦 Версия <code>{c.VERSION}</code>\n"
            f"👤 <code>{c.account.username}</code> · ID <code>{c.account.id}</code>\n"
            f"💰 <code>{c.balance.total_rub}₽</code> · <code>{c.balance.total_usd}$</code> · "
            f"<code>{c.balance.total_eur}€</code>\n"
            f"📊 Активные заказы: <code>{c.account.active_sales}</code>\n\n"
            "⚙️ Панель: <code>/menu</code>\n"
            "🔄 @funpayplata · 🧩 @quantumdeals · 👨‍💻 @hambinoww")
...
BIND_TO_ORDER_STATUS_CHANGED = [update_plata_analytics_handler, send_thank_u_message_handler,
