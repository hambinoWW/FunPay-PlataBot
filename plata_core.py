...
from Utils.plata_tools import validate_proxy, build_proxy
...
from plata_modules import confirm_reminder, old_orders, review_reply, profile_stats
...
from plata_identity import PRODUCT_NAME
...
def get_plata() -> None | Plata:
    """
    Возвращает основной экземпляр PLATA.
    """
    return getattr(Plata, "primary_instance", None)
...
class Plata(object):
    def __init__(self, main_config: ConfigParser,
                 ...
                 version: str):
        if not hasattr(Plata, "primary_instance"):
            Plata.primary_instance = self
        self.VERSION = version
        ...
        # Прокси
        self.proxy = {}
        self.proxy_dict = plata_tools.load_proxy_dict()  # прокси {0: "login:password@ip:port", 1: "ip:port"...}
        if self.MAIN_CFG["Proxy"].getboolean("enable"):
            if self.MAIN_CFG["Proxy"]["proxy"]:
                ...
                if proxy_str not in self.proxy_dict.values():
                    ...
                    plata_tools.cache_proxy_dict(self.proxy_dict)

                if self.MAIN_CFG["Proxy"].getboolean("check") and not plata_tools.check_proxy(self.proxy):
                    sys.exit()
        ...
        self.blacklist = plata_tools.load_blacklist()  # ЧС.
        self.old_users = plata_tools.load_old_users(
        ...
        self.disabled_plugins = plata_tools.load_disabled_plugins()
        self.pinned_plugins = plata_tools.load_pinned_plugins()

    def __init_account(self, attempts: int | None = None) -> bool:
        """
    ...
    def raise_lots(self) -> int:
        ...
        for subcat in sorted(list(self.curr_profile.get_sorted_lots(2).keys()), key=lambda x: x.category.position):
            ...
            try:
                ...
                time_delta = "" if not last_time else f" Последнее поднятие: {plata_tools.time_to_str(new_time - last_time)} назад."
                error_text = f"Подождите {plata_tools.time_to_str(wait_time)}."
                ...
                if e.wait_time is not None:
                    logger.warning(_("crd_raise_time_err", subcat.category.name, error_text,
                                     plata_tools.time_to_str(e.wait_time)))
    ...
    def parse_message_entities(self, msg_text: str) -> list[str | int | float]:
        ...
        pos = 0
        entities = []
        while entity := plata_tools.ENTITY_RE.search(msg_text, pos=pos):
    ...
    def init(self, account_attempts: int | None = None):
        """
        ...
        for module in (confirm_reminder, old_orders, review_reply, profile_stats):
        ...
        # The fallback callback handler must be registered last. Cardinal
        # plugins register their settings callbacks during PRE_INIT; placing
        # the fallback before them intercepts PLUGIN_SETTINGS callbacks.
        if self.MAIN_CFG["Telegram"].getboolean("enabled"):
            for handler in getattr(default_cp, "BIND_TO_PRE_INIT", []):
                handler(active_cardinal)
        ...
        if not self.__init_account(account_attempts):
            raise RuntimeError(f"FunPay account initialization failed: {getattr(self, 'account_profile_id', 'unknown')}")
        self.runner = FunPayAPI.Runner(self.account, self.old_mode_enabled)
        if not self.__update_profile(infinite_polling=account_attempts is None, attempts=account_attempts or 0):
            raise RuntimeError(f"FunPay profile initialization failed: {getattr(self, 'account_profile_id', 'unknown')}")
    ...
    def product_path(self, file_name: str) -> str:
        return os.path.join(getattr(self, "products_directory", "storage/products"), file_name)
    ...
    def save_config(self, config: configparser.ConfigParser, file_path: str) -> None:
        ...
        """
        if config is self.MAIN_CFG:
            file_path = getattr(self, "main_config_path", file_path)
        elif config is self.AD_CFG:
            file_path = getattr(self, "auto_delivery_config_path", file_path)
        elif config is self.RAW_AR_CFG or config is self.AR_CFG:
            file_path = getattr(self, "auto_response_config_path", file_path)
    ...
    def run_handlers(self, handlers_list: list[Callable], args) -> None:
        ...
        for func in handlers_list:
            try:
                ...
                account_id = getattr(self, "account_profile_id", "primary")
                if plugin_uuid is None or (plugin_uuid in self.plugins and self.plugins[plugin_uuid].enabled and
    ...
    def toggle_plugin(self, uuid):
        ...
        plata_tools.cache_disabled_plugins(self.disabled_plugins)
    ...
    def pin_plugin(self, uuid):
        ...
        plata_tools.cache_pinned_plugins(self.pinned_plugins)
...
# Legacy plugin API references the exact same PLATA class.
Cardinal = Plata
